...
## Potts filtering on compact array;
	...
	## sq: sum of squares of values between breakpoints
      ...
      #Return now as the if statement will fail if cost is NA
      if (is.na(cost)) {
        return(NA)
      }
      if (totCost < cost) {
...
## filters (L>L2), then by a filter seaching for potential kmin long segments
  ...
  #Should check that we have a minimum number of x values
  #print(paste("number of x=", length(x), " min_size=", (6*L+1)))
  xc<-cumsum(x)
  xc<-c(0,xc)
