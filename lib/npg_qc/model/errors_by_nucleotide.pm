...
# Id:            $Id: errors_by_nucleotide.pm 8943 2010-03-30 15:40:28Z jo3 $
# Source:        $Source: /repos/cvs/webcore/SHARED_docs/cgi-bin/docrep,v $
# $HeadURL: svn+ssh://intcvs1.internal.sanger.ac.uk/repos/svn/new-pipeline-dev/npg-qc/trunk/lib/npg_qc/model/errors_by_nucleotide.pm $
#

package npg_qc::model::errors_by_nucleotide;
use strict;
use warnings;
...
sub init {
  ...
  if($self->id_run_tile() &&
     ...
     !$self->id_errors_by_nucleotide()) {

    my $query = q(SELECT id_errors_by_nucleotide
                 FROM errors_by_nucleotide
                 WHERE id_run_tile = ?
                 AND read_as = ?
                 AND rescore = ?
                 );
    ...
    if(@{$ref}) {
      $self->id_errors_by_nucleotide($ref->[0]->[0]);
    }
  }
  return 1;
}
...
=head1 SYNOPSIS
  ...
  my $oErrorsByNucleotide = npg_qc::model::errors_by_nucleotide->new({util => $util});
...
=head2 fields - return array of fields, first of which is the primary key
  ...
  my $aFields = $oErrorsByNucleotide->fields();

=head2 run_tile - returns run_tile object that this object belongs to

  my $oRunTile = $oErrorsByNucleotide->run_tile();
