...
sub init {
  ...
  if($self->id_run_tile() &&
     ...
     !$self->id_errors_by_cycle_and_nucleotide()) {

    my $query = q(SELECT id_errors_by_cycle_and_nucleotide
                 FROM errors_by_cycle_and_nucleotide
                 WHERE id_run_tile = ?
                 AND cycle = ?
                 AND read_as = ?
                 AND rescore = ?
    ...
    if(@{$ref}) {
      $self->id_errors_by_cycle_and_nucleotide($ref->[0]->[0]);
    }
  }
  return 1;
}
...
=head1 SYNOPSIS
  ...
  my $oErrorByCycleAndNucleotide = npg_qc::model::errors_by_cycle_and_nucleotide->new({util => $util});
...
=head2 fields - return array of fields, first of which is the primary key
  ...
  my $aFields = $oErrorByCycleAndNucleotide->fields();

=head2 run_tile - returns run_tile object that this object belongs to

  my $oRunTile = $oErrorByCycleAndNucleotide->run_tile();
