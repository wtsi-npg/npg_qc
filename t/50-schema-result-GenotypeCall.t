...
use_ok('npg_qc::Schema::Result::GenotypeCall');


my $schema = Moose::Meta::Class->create_anon_class(
          roles => [qw/npg_testing::db/])
          ->new_object({})->create_test_db(q[npg_qc::Schema]);
my $archive = q[t/data/autoqc/genotype_call/data];
my $rs = $schema->resultset('GenotypeCall');
...
subtest 'result with criterion in json' => sub {
  ...
  my $values = _get_data('24135_1#1.genotype_call.json');
  $values->{'id_seq_composition'} =
    t::autoqc_util::find_or_save_composition($schema,
    {id_run => 24135, position => 1, tag_index => 1});
  my %values1 = %{$values};
  my $v1 = \%values1;
  ...
  is($row->criterion, q[Genotype passed rate >= 0.7],
...
subtest 'load results with a composition fk' => sub {
  ...
  my $values =  _get_data('24135_1#1.genotype_call.json');
  my $fk_row = $schema->resultset('SeqComposition')->create({digest => '24135', size => 2});

  my $object = $rs->new_result($values);
  isa_ok($object, 'npg_qc::Schema::Result::GenotypeCall');
  throws_ok {$object->insert()}
    qr/NOT NULL constraint failed: genotype_call.id_seq_composition/,
