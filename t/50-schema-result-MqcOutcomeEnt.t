...
subtest 'Misc tests' => sub {
  ...
  $object->update_outcome({'mqc_outcome' => 'Rejected final'}, 'new_user', 'RT#356789');
  ...
  throws_ok {$object->update_outcome({'mqc_outcome' => 'some invalid'}, 'user')}
    qr/Outcome some invalid is invalid/,
    'error updating to an invalid string outcome';
  throws_ok {$object->update_outcome({'mqc_outcome' => 123}, 'user')}
    qr/Outcome 123 is invalid/,
    'error updating to invalid integer status';
  throws_ok {$object->update_outcome({'mqc_outcome' => 'Rejected final'})}
...
subtest q[update on a new result] => sub {
  ...
  $new_row = $rs->new_result($args);
  $new_row->update_outcome({mqc_outcome => $outcome}, 'dog', 'cat');
