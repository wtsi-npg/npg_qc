...
do
  for qc_set in ${plex[*]}
    ...
    printf "Generating data_object list for  gbs_plex = %s\n" ${qc_set}   
    jq -n "{avus: [{attribute: \"gbs_plex\", value: \"${qc_set}\"}]}" | (irodsEnvFile=$HOME/.irods/.irodsEnv-${zone}_gtck baton-metaquery --zone ${zone} --unbuffered) | jq . | egrep -v "^\[|\]$" | sed -e "s/^  \},$/  \}/" > fluidigm_${qc_set}_${zone}_baton_plex_list_${dttag}.txt
    else
